...
# The Cargo workspace version is shared, but each binding is released on its own
# cadence — a swift-v$VERSION tag may not exist yet. The site serves whatever zip
# is baked into /downloads (its single source of truth), so when the exact tag is
# missing, fall back to the latest published swift-v* release instead of
# hard-failing the deploy. A real swift-v$VERSION is picked up once published.
if command -v gh >/dev/null 2>&1 && ! gh release view "$TAG" --repo "$REPO" >/dev/null 2>&1; then
  latest="$(gh release list --repo "$REPO" --json tagName,createdAt \
    -q 'map(select(.tagName|startswith("swift-v"))) | sort_by(.createdAt) | last | .tagName' 2>/dev/null || true)"
  if [ -n "$latest" ] && [ "$latest" != "null" ]; then
    printf '\033[1;33m   aviso: release %s não existe; usando o último publicado: %s\033[0m\n' "$TAG" "$latest"
    VERSION="${latest#swift-v}"
    TAG="$latest"
    PKG_ZIP="rustpdf-swift-${VERSION}.zip"
    XCF_ZIP="RustPdfFFI-${VERSION}.xcframework.zip"
  fi
